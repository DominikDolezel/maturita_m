\section{Komplexní čísla}
\begin{definition}\label{kompl_c_def}
Množinu označenou $\mathbb C = \left \{ (a,b);  a,b \in \mathbb R \right \} $
spolu s operacemi $+$ a $\,\cdot\,$ definovanými
\begin{enumerate}[$i.$]
\item $(a,b) + (c,d) = (a+c, b+d),$
\item $(a,b) \cdot (c,d) = (ac-bd, ad+bc)$,
\end{enumerate}
nazýváme \textbf{komplexními čísly}.
\end{definition}

\begin{veta}
    Pro všechna $x=(a,b),y=(c,d) \in \mathbb C$ platí
    \begin{enumerate}[$i.$]
    \item $x-y=(a-c, b-d)$,
   	\item $y\ne (0,0)\colon  \frac{x}{y}=\left ( \frac{ac+bd}{c^2+d^2}, \frac{bc-ad}{c^2+d^2} \right ) .$
    \end{enumerate}
\end{veta}

\begin{veta}
    Množina $\mathbb C$ spolu s operacemi $+,\,\cdot$ tvoří komutativní těleso.
\end{veta}

\begin{pozn}\label{teleso}
    Množina $M$ spolu s operacemi $+,\cdot$ tvoří (komutativní) těleso, pokud
    \begin{enumerate}[$i.$]
    \item $(M,+)$ je komutativní grupa,
   	\item $(M\smallsetminus\left \{ 0 \right \} ,\,\cdot\,)$ je komutativní grupa,
   	\item platí distributivní zákony.
    \end{enumerate}
    Tělesem je třeba $\mathbb R, \mathbb Q;$ $\mathbb Z$ však tělesem není (neexistují
    inverzní prvky vzhledem k násobení, \textit{Pět krát co je jedna?}).
\end{pozn}

\begin{veta}
    Množiny $\mathbb R$ a $\{(x,0); x\text{ lib.}\} \subset \mathbb C$ jsou izomorfní.
\end{veta}

\begin{proof}
    Pro všechna $x,y \in \mathbb R$ platí
    \begin{enumerate}[$i.$]
    \item $\varphi(x)+\varphi(y) = \varphi(x+y),$
   	\item $\varphi(x)\cdot\varphi(y) = \varphi(xy),$
    \end{enumerate}
    kde $\varphi$ je zřejmě bijektivní zobrazení z $\mathbb R$ do $\mathbb C$: $\varphi(a)=(a,0).$
\end{proof}

\begin{pozn}
    Dále budeme zapisovat komplexní číslo $(a,b)$ jako $a+bi.$
\end{pozn}

\begin{definition}
    Tvar komplexního čísla $a+bi$ nazýváme \textbf{algebraický}. Číslo $a$ je jeho
   \textbf{reálná část}, $b$ \textbf{imaginární část}.\\
  Číslo $(0,1)$ označme $i$ a nazýváme \textbf{imaginární jednotkou}. \\
  Čísla $a+bi,$ kde $b\ne 0$ nazýváme \textbf{imaginárním}. Pokud také $a=0$,
  nazýváme je \textbf{ryze imaginárním}.
\end{definition}

\begin{pozn}
    Komplexní číslo $x=(a,b)$ lze chápat jako bod v rovině se souřadnicemi $[a,b].$
\end{pozn}

\begin{definition}
    Nechť je dáno komplexní číslo $x= a+bi.$ Číslo $\overline{x}=a-bi$ nazýváme
   \textbf{komplexně sdruženým} k číslu $x$. Číslo $|x|=\sqrt{a^2+b^2}$ je
   \textbf{absolutní hodnota} čísla $x$. Jestliže $|x|=1,$ číslu $x$ říkáme \textbf{komplexní jednotka}.
\end{definition}

\begin{definition}
\textbf{Argumentem} komplexního čísla nazýváme orientovaný úhel $\varphi$, který svírá kladná
poloosa reálné osy s polohovým vektorem daného bodu představujícím dané komplexní číslo.
\end{definition}

\begin{veta}
    Každé komplexní číslo $x=a+bi, x\ne 0,$ lze zapsat ve tvaru
    $$x=|x|(\cos \varphi + i \sin \varphi),$$
    kde $\varphi$ je argumentem čísla $x$. Dále platí:
    \begin{align*}
        \cos \varphi = \frac{a}{|x|}, && \sin \varphi = \frac{b}{|x|}.
    \end{align*}
\end{veta}

\begin{definition}
    Tvar komplexního čísla $x= |x|(\cos \varphi + i\sin\varphi)$ nazýváme \textbf{goniometrický}.
\end{definition}

\begin{pozn}
    Každé komplexní číslo lze zapsat uspořádanou dvojicí $x=(|x|, \varphi).$
    Tato čísla jsou \textbf{polárními souřadnicemi} komplexního čísla $x$.
\end{pozn}

\begin{veta}[Moivreova věta]
    Nechť je dáno komplexní číslo $x=|x|(\cos \varphi + i \sin \varphi)$ a $n\in \mathbb N$
    je nenulové číslo. Pak platí
    $$x^n = |x|^n (\cos n\varphi + i \sin n \varphi).$$
\end{veta}

\begin{pozn}
    Gaussova rovina je množina bodů $[a,b]; a,b \in \mathbb R$.
    Každému prvku této množiny odpovídá komplexní číslo $(a,b).$ Osa $x$ odpovídá
    reálné části tohoto komplexního čísla, osa $y$ imaginární části.
\end{pozn}

\begin{priklad}
Jsou dána komplexní čísla $a=2+i$, $b=1-2i$. Zobrazte v Gaussově rovině jejich součin a podíl.
\end{priklad}

\begin{reseni}
Je
$$
    a \cdot b = (2+i) \cdot (1-2i) = 4-3i.
$$
U podílu využejeme následujícího triku:
$$
\frac{a }{b} = \frac{2+i}{1-2i}\cdot \frac{1+2i}{1+2i} = \frac{5i}{5} = i.
$$
Body jsou zobrazeny v Gaussově rovině na obrázku \ref{komplcpr}.
\begin{figure}[h]
	\centering
	\begin{tikzpicture}
 % Osy
    \draw[->, thick] (-1.5,0) -- (5,0) node[right] {$\text{Re}$};
    \draw[->, thick] (0,-4) -- (0,2) node[above] {$\text{Im}$};

    % Popisky os a mřížka
    \foreach \x in {-1,1,2,3,4}
        \draw[gray!30, dashed] (\x,-3.5) -- (\x,1.5);
    \foreach \y in {-3,-2,-1,1}
        \draw[gray!30, dashed] (-1,\y) -- (4.5,\y);

    % Souřadnice bodů
    \coordinate (A) at (2,1);
    \coordinate (B) at (1,-2);
    \coordinate (P) at (4,-3); % Součin a*b
    \coordinate (Q) at (0,1); % Podíl a/b

    % Vykreslení vektorů / bodů
    \draw[blue, thick, -stealth] (0,0) -- (A) node[above right] {$a = 2+i$};
    \draw[blue, thick, -stealth] (0,0) -- (B) node[below right] {$b = 1-2i$};
    \draw[red, thick, -stealth] (0,0) -- (P) node[right] {$ab = 4-3i$};
    \draw[red, thick, -stealth] (0,0) -- (Q) node[above left] {$\frac{a}{b} = i$};

    % Zvýraznění bodů
    \fill[blue] (A) circle (2pt);
    \fill[blue] (B) circle (2pt);
    \fill[red] (P) circle (2pt);
    \fill[red] (Q) circle (2pt);
	\end{tikzpicture}
	\caption{Zobrazení $a$, $b$ a jejich součinu a podílu v Gaussově rovině}
	\label{komplcpr}
\end{figure}
\end{reseni}

\begin{definition}
(\textbf{Algebraická}) \textbf{rovnice v komplexní neznámé} $n$-tého stupně s~jednou neznámou $x\in \mathbb C$ je každá
rovnice tvaru $P(x)=0,$ kde $P(x)$ je polynom $n$-tého stupně s~reálnými (resp.
komplexními) koeficienty.
\end{definition}

\begin{veta}[Základní věta algebry]
    Každý polynom $n$-tého stupně s komplexními koeficienty má v množině $\mathbb C$
    právě $n$ kořenů, počítáme-li každý kořen tolikrát, kolik je jeho násobnost.
\end{veta}

\begin{veta}
    Má-li algebraická rovnice s reálnými koeficienty imaginární kořen
    $x = a+bi$, má také kořen $\overline{x} = a-bi.$
\end{veta}

\begin{definition}
\textbf{Binomickou rovnicí} s neznámou $x\in \mathbb C$ nazýváme každou rovnici
tvaru $x^n=a$, kde $a\in \mathbb C,n\in \mathbb N, n\geq 2.$ Každý komplexní
kořen binomické rovnice nazýváme \textbf{komplexní $n$-tou odmocninou} z čísla $a.$
\end{definition}

\begin{priklad}\label{prikladkompl}
Vypočtěte $z=\sqrt{1+\sqrt{3}i }. $
\end{priklad}

\begin{reseni}
Umocněním získáme $z^2=1+\sqrt{3}i. $ Platí $|z^2|=2,$ takže
$$z^2=|z|^2(\cos 2x+i\sin 2x)=2\left (\cos \frac{\pi}{3}+i\sin \frac{\pi}{3}\right).$$
Metodou porovnání koeficientů dostaneme
\begin{align*}
	\cos 2x = \cos \frac{\pi}{3}, && \sin 2x = \sin \frac{\pi}{3}.
\end{align*}
Řešením první rovnice je množina
$$
\left \{ \pm \frac{\pi}{6} + k\pi, k \in \mathbb Z  \right \},
$$
řešením druhé rovnice pak
$$
\left \{ \frac{\pi}{6} + k\pi, k \in \mathbb Z  \right \} \cup \left \{ \frac{\pi}{3} + l\pi, l \in \mathbb Z  \right \} ,
$$
odkud dostáváme, že
$$
    x \in \left \{ \frac{\pi}{6} + k\pi, k \in \mathbb Z \right \}.
$$
Je tedy
\begin{align*}
z = |z|\left( \cos x + i\sin x \right ) &= \sqrt{2} \left( \cos \frac{\pi}{6} + i \sin \frac{\pi}{6} \right ) = \sqrt{2}\left (\frac{\sqrt{3}}{2} + i\frac{1}{2}\right), \\
&= \sqrt{2} \left( \cos \frac{7\pi}{6} + i \sin \frac{7\pi}{6} \right ) =  \sqrt{2} \left( -\frac{1}{2} + i \frac{\sqrt{3}}{2} \right ).
\end{align*}
\end{reseni}

\begin{priklad}
Řešte v $\mathbb C$ binomickou rovnici $z^3=i.$
\end{priklad}

\begin{reseni}
Vyjádříme $i$ v goniometrickém tvaru. Je
$$
    z^3 = i = \cos \frac{\pi}{2} + i\sin \frac{\pi}{2} = \cos 3x + i \sin 3x.
$$
Opět máme dvojici rovnic
\begin{align*}
	\cos \frac{\pi }{2} = \cos 3x, && \sin \frac{\pi }{2} = \sin 3x.
\end{align*}
Řešením první rovnice je množina
$$
\left \{ \pm \frac{\pi }{6} + \frac{2}{3} k \pi, k \in \mathbb Z  \right \} .
$$
Řešením druhé rovnice je
$$
\left \{ \frac{\pi}{6} +\frac{2}{3} k\pi, k \in \mathbb Z  \right \} .
$$
Celkem je pak
$$
    x \in \left \{ \frac{\pi}{6} + \frac{2}{3} k \pi, k \in \mathbb Z  \right \} .
$$
Dostáváme tedy, že
\begin{align*}
	z =  \cos x + i \sin x &= \cos \frac{\pi}{6} + i \sin \frac{\pi }{6} = \frac{\sqrt{3}}{2} + i \frac{1}{2}, \\
	&= \cos \frac{5\pi }{6} + i \sin \frac{5\pi }{6} = -\frac{\sqrt{3}}{2} + i \frac{1}{2},\\
	&= \cos \frac{9\pi}{6} + i \sin \frac{9\pi }{6} = 0 - i.
\end{align*}
\end{reseni}

\begin{definition}
\textbf{Kvadratickou rovnicí s} (komplexní) \textbf{neznámou} $x\in \mathbb C$ a
\textbf{reálnými} (resp. \textbf{komplexními}) \textbf{koeficienty} $a,b,c$
nazýváme každou rovnici tvaru
$ax^2+bx+c=0,$ $a,b,c\in \mathbb R,a\ne0$ (resp. $a,b,c \in \mathbb C,a\ne 0$).
\end{definition}

\begin{priklad}
V $\mathbb C$ řešte $z^2+3z+10i=0.$
\end{priklad}

\begin{reseni}
\begin{enumerate}[1.]
\item způsob: Doplněním na čtverec dostáváme $\left ( z+\frac{3}{2} \right )^2=\frac{9}{4}-10i. $
Zavedeme substituci $y=z + \frac{3}{2}$ a dále řešíme jako v příkladu \ref{prikladkompl}.
\item způsob: Pomocí diskriminantu. Je
$$
    D = 3^2 - 4 \cdot 10i = 9 - 40i.
$$
Nyní musíme spočítat $\sqrt{9-40i}$, abychom mohli použít vzorec. Položme
$$
\sqrt{9-40i} = a+bi.
$$
Pak
$$
9-40 i = a^2 + 2abi - b^2.
$$
Dostáváme soustavu dvou rovnic
\begin{align*}
	9 = a^2 - b^2, && -40 = 2ab,
\end{align*}
která má řešení $\pm 5 \mp 4i$. Dosadíme do vzorce a dostaneme
\begin{align*}
z_{1,2} = \frac{-3 \pm (5-4i)}{2} &= 1-2i,\\
&= -4+2i.
\end{align*}
\item způsob: Vyjádřením neznámé v algebraickém tvaru ($z=a+bi$) a roznásobením. Dostaneme
$$
    a^2 + 2abi -b^2 + 3a + 3bi + 10 i = 0.
$$
Metodou porovnání koeficientů dostaneme soustavu
\begin{align*}
	a^2 - b^2 + 3a =0, && 2ab + 3b+10 = 0,
\end{align*}
kterou vyřešíme v $\mathbb R $.
\end{enumerate}
\end{reseni}

\begin{definition}
\textbf{Trinomická rovnice} s neznámou $x\in \mathbb C$ a reálnými koeficienty $a,b,c \in \mathbb R$
nazýváme rovnici tvaru $ax^p + bx^q + c = 0,$ kde $p,q\in \mathbb N, p>q, a,b\ne 0.$
\end{definition}

\begin{priklad}
Užitím Moivreovy věty odvoďte vzorce pro $\sin 3x, \cos 3x.$
\end{priklad}

\begin{reseni}
Platí $(\cos x + i\sin x)^3=\cos 3x+i\sin 3x.$ Z binomické věty máme, že
$$(\cos x+i\sin x)^3=\cos^3 x + 3i\cos^2 x\sin x - 3\cos x\sin^2 x-i\sin^3 x.$$
Metodou porovnání koeficientů u $i$ vzorce dostáváme.
\end{reseni}
