...
\begin{pozn}
V této kapitole se snažíme zjistit, kolik \textit{možností} (jak něco seřadit, poskládat, vybrat apod.) může dle daných kritérií existovat.
\end{pozn}
...
\begin{pozn}[Kombinatorický význam pravidla součtu]
	Když si rozdělíme jev na případy, které nemohou nastat zaráz (tj. pro nástup jevu se musí stát buď to, nebo ono), četnost jevu se rovná součtu četností jednotlivých případů.
\end{pozn}
...
\begin{pozn}[Kombinatorický význam pravidla součinu]
	Když si jev rozdělíme na podmínky, které musí nastat zaráz (tj. pro nástup jevu se musí stát to a zároveň ono), četnost jevu se rovná součinu četností splění jednotlivých podmínek.
\end{pozn}
...
\begin{veta}\label{vfakt}
   ...
   P(n)=n\cdot(n-1)\cdot(n-2)\cdot\hdots\cdot1.
    \end{equation}
\end{veta}

\begin{definition}
    Výraz \ref{fakt} z věty \ref{vfakt} značíme
    $$n\cdot(n-1)\cdot(n-2)\cdot\hdots\cdot1=n!$$
...
\begin{veta}
    ...
    $$P_o(n_1, n_1, \dots, n_d)=\frac{n!}{n_1!\cdot n_2!\cdot\hdots \cdot n_d!}=
...
\begin{definition}
    ...
    vytvořenou z prvků těchto $d$ druhů. Počet těchto variací označme $V_o(k,d).$
\end{definition}

\begin{veta}
    Pro všechna $k, d \in \mathbb N$ platí
    $$V_o(k,d)=d^k.$$
\end{veta}
...
\begin{veta}\label{binomvztahy}
    Pro všechna $ k,n \in \mathbb N_0, k\leq n$ platí
    \begin{align*}
...
\begin{reseni}
    Podle definice kombinačního čísla máme rovnici
    $$
    \frac{(x-1)!}{2!(x-3)!} + \frac{(x-2)!}{2!(x-4)!} = 9.
    $$
    Členy s faktoriálem se ale vykrátí. Dostáváme
    $$
    \frac{(x-1)(x-2)}{2} + \frac{(x-2)(x-3)}{2} = 9.
    $$
    Tuto rovnici vyřešíme standardním způsobem, dostaneme řešení $x = 5$ a $x=-1$. Druhé zmíněné ale není definované.
    Řešením je tedy $x=5$.
\end{reseni}

\begin{definition}
    Nechť je dáno $n_1$ prvků prvního druhu, $n_2$ prvků druhého druhu, \dots,
    $n_d$ prvků $d$-tého druhu. Nechť $k\in \mathbb N\colon k\leq n_i, \forall i
...
\begin{veta}\label{sifra}
    Pro všechna $k,d \in \mathbb N$ platí
    $$K_o(k,d)=P_o(k,d-1)=\binom{k+d-1}{d-1}.$$
\end{veta}

\begin{pozn}
    Význam věty \ref{sifra} je vysvětlen v následujícím příkladu.
\end{pozn}
...
\begin{reseni}
Pomocí pravidla součtu a pravidla součinu rozložíme na jednoduše vyčíslitelné podpříklady.
\begin{enumerate}[$a.$]
\item Je to
...
Vyjde
$$
\binom{15}{2} \cdot \binom{20}{4} + \binom{15}{1} \cdot \binom{20}{5} + \binom{15}{0} \cdot \binom{20}{6}=  780045.
$$
...
jedna dívka. Vyjde
$$
    \binom{35}{6} - \binom{15}{1} \cdot \binom{20}{5} - \binom{15}{0} \cdot \binom{20}{6} =1351840.
$$
\end{enumerate}
\end{reseni}

\begin{veta}[Binomická věta]\label{binom_v}
...
\begin{pozn}[Pascalův trojúhelník]
    ...
    \begin{gather*}
        ...
        1\vphantom{\binom{a}{a}} \quad\quad 1 \\
        1\vphantom{\binom{a}{a}} \quad\quad 2 \quad\quad 1 \\
        1\vphantom{\binom{a}{a}} \quad\quad 3 \quad\quad 3 \quad\quad 1 \\
        1\vphantom{\binom{a}{a}} \quad\quad 4 \quad\quad 6 \quad\quad 4 \quad\quad 1 \\
        1\vphantom{\binom{a}{a}} \quad\quad 5 \quad\quad 10 \quad\quad 10 \quad\quad 5 \quad\quad 1 \\
...
\begin{priklad}
Určete absolutní člen v rozvoji výrazu $\left(2x^2-\frac{3}{x}\right)^6$.
\end{priklad}

\begin{reseni}
Použijeme binomickou větu (větu \ref{binom_v}).
...
soustava dvou rovnic o dvou neznámých, kterou vyřešíme třeba sečtením obou rovnic. Dostaneme
$3i=6$, tedy $i=2$. Odtud z druhé rovnice $j=4$. Absolutní člen v~rozvoji výrazu $\left(2x^2-\frac{3}{x}\right)^6$
je tedy
$$
    \left (2x^2\right)^2 \left(\frac{3}{x}\right)^4=\frac{4x^4 \cdot 81}{x^4}=324.
$$
\end{reseni}
...
\begin{pozn}
	Je-li dáno $k$ konečných množin $M_1, M_2, M_3, \dots, M_k$, vyjádříme počet prvků jejich sjednocení pomocí počtů prvků množin a jejich průniků. Pro $k=2$ a $k=3$ můžeme tento vztah vypozorovat z Vennova diagramu a dále jej zobecnit pro libovolné $k$.
	\begin{align*}
	k=2\colon |M_1\cup M_2| &= |M_1|+|M_2|-|M_1 \cap M_2|,\\
	k=3\colon |M_1\cup M_2 \cup M_3| &= |M_1|+|M_2|+|M_3|-|M_1 \cap M_2|-|M_2 \cap M_3|-|M_3\cap M_1|\\&+|M_1 \cap M_2 \cap M_3|.
	\end{align*}
\end{pozn}

\begin{veta}[Princip inkluze a exkluze]
    Nechť $M_1, M_2, M_3, \dots, M_k$ jsou konečné množiny. Pak platí
    $$
        \left | \bigcup\limits_{i=1}^k M_i \right | = \sum_{j=1}^k (-1)^{j+1} \sum_{I \in \binom{[n]}{j}} \left |\bigcap_{i \in I} A_i \right|,
    $$
    kde $\binom{[n]}{j}$ značí všechny možné $j$-tice z prvků $1$ až $n$.
\end{veta}
...
\begin{reseni}
Odvodíme rekurentní vztah. Je dán $n$-úhelník a zjistíme, kolik přibude
