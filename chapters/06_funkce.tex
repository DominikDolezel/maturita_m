...
\begin{definition}
  ...
  \[
    \mathcal D(f) = \left  \{ x \in \mathbb R:\exists ! y \in \mathbb R:y=f(x) \right \}
  \]
  nazveme \textbf{definičním oborem} funkce $f$. Množinu
  \[
    \mathcal H(f) = \left  \{ y \in \mathbb R:\exists  x \in \mathbb R:y=f(x) \right \}
  \]
...
\begin{definition}
	Nechť $f:\mathcal D(f)\to \mathcal H(f)$ je prostá funkce. \textbf{Inverzní funkcí} k funkci $f$ nazveme funkci
	$f^{-1}:\mathcal H(f)\to \mathcal D(f)$ takovou, že
	$$
	    f(x)=y\iff f^{-1}(y)=x.
	$$
\end{definition}
...
\begin{definition}\label{lichasuda}
  ...
  \begin{enumerate}[$i.$]
    \item $\forall x \in \mathcal D(f): -x \in \mathcal D(f)$ a
  	\item $\forall x \in \mathcal D(f): f(-x)=-f(x)$, resp. $f(-x)=f(x)$.
  \end{enumerate}
\end{definition}
...
\begin{definition}
  Nechť $f$ je funkce. Funkce $f$ se nazývá \textbf{periodická}, pokud $\exists p \in \mathbb R^{+}: \forall x \in \mathcal D(f):$
  \begin{enumerate}
...
\begin{definition}
  Nechť máme funkci $f: y = f(u)$ s definičním oborem $\mathcal D(f)$ a funkci $g: u=g(x)$ s oborem hodnot $\mathcal H(g)$. Jestliže je $\mathcal H(g) \subseteq \mathcal D(f)$, pak funkci $h: y = f(g(x))$ nazveme \textbf{složenou funkcí} (někdy píšeme též $h=f \circ g$).
\end{definition}
