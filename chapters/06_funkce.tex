...
\begin{definition}
  ...
  \[
    \mathcal D(f) = \left  \{ x \in \mathbb R;\exists ! y \in \mathbb R\colon y=f(x) \right \}
  \]
  nazveme \textbf{definičním oborem} funkce $f$. Množinu
  \[
    \mathcal H(f) = \left  \{ y \in \mathbb R;\exists  x \in \mathbb R\colon y=f(x) \right \}
  \]
...
\begin{pozn}
  \textbf{Grafem funkce} rozumíme množinu všech bodů $[x,f(x)]$, kde $x\in \mathcal D(f).$
\end{pozn}
...
\begin{definition}
  ...
  \[
    \forall x_1,x_2\in \mathcal D(f)\colon x_1\ne x_2 \implies f(x_1)\ne f(x_2).
  \]
\end{definition}

\begin{definition}
	Nechť $f\colon \mathcal D(f)\to \mathcal H(f)$ je prostá funkce. \textbf{Inverzní funkcí} k funkci $f$ nazveme funkci
	$f^{-1}\colon \mathcal H(f)\to \mathcal D(f)$ takovou, že
	$$
	    f(x)=y\iff f^{-1}(y)=x.
	$$
\end{definition}
...
\begin{priklad}
K funkci $f\colon y=2x-1$ určete $f^{-1}$ a načrtněte grafy obou funkcí.
\end{priklad}
...
\begin{definition}\label{lichasuda}
  Funkce $f$ se nazývá \textbf{lichá} (resp. \textbf{sudá}), jestliže zároveň platí
  \begin{enumerate}[$i.$]
    \item $\forall x \in \mathcal D(f)\colon -x \in \mathcal D(f)$,
  	\item $\forall x \in \mathcal D(f)\colon f(-x)=-f(x)$ (resp. $f(-x)=f(x)$).
  \end{enumerate}
\end{definition}
...
\begin{enumerate}[$i.$]
\item $\mathcal D(f)=\mathbb R$ -- platí,
\item $f(-x)=(-x)^2=x^2=f(x)$ -- platí, tedy funkce je sudá.
\end{enumerate}
\end{reseni}

\begin{definition}
  Nechť $f$ je funkce a $M$ alespoň dvouprvková množina z $\mathcal D(f)$. Řekneme, že funkce $f$ je na množině $M$
  \begin{enumerate}[$i.$]
    \item \textbf{rostoucí} $\iff \forall x_1, x_2 \in M\colon x_1 < x_2 \implies f(x_1) < f(x_2),$
    \item \textbf{klesající} $\iff \forall x_1, x_2 \in M\colon x_1 < x_2 \implies f(x_1) > f(x_2),$
    \item \textbf{neklesající} $\iff \forall x_1, x_2 \in M\colon x_1 < x_2 \implies f(x_1) \leq f(x_2),$
    \item \textbf{nerostoucí} $\iff \forall x_1, x_2 \in M\colon x_1 < x_2 \implies f(x_1) \geq f(x_2).$
  \end{enumerate}
...
\begin{definition}
  Nechť $f$ je funkce, $M\subseteq \mathcal D(f)$. Řekneme, že funkce $f$ je na množině $M$
  \begin{enumerate}[$i.$]
    \item \textbf{shora omezená} $\iff \exists k \in \mathbb R\colon \forall x \in M\colon f(x)\leq k,$
    \item \textbf{zdola omezená} $\iff \exists k \in \mathbb R\colon \forall x \in M\colon f(x)\geq k,$
    \item \textbf{omezená} $\iff $ je zdola i shora omezená.
  \end{enumerate}
\end{definition}

\begin{definition}
  Nechť $f$ je funkce, $M \subseteq \mathcal D(f)$, v ní prvek $a \in M$.
  Řekneme, že funkce $f$ má v~bodě $a$:
  \begin{enumerate}[$i.$]
    \item \textbf{ostré maximum} na množině $M$ právě tehdy, když $\forall x \in M\smallsetminus \left \{ a \right \}  \colon f(x) < f(a)$,
    \item \textbf{maximum} (neostré) na množině $M$ právě tehdy, když $\forall x \in M \smallsetminus \left \{ a \right \}\colon f(x) \leq f(a)$,
    \item \textbf{ostré minimum} na množině $M$ právě tehdy, když $\forall x \in M\smallsetminus \left \{ a \right \}\colon f(x) > f(a)$,
    \item \textbf{minimum} (neostré) na množině $M$ právě tehdy, když $\forall x \in M\smallsetminus \left \{ a \right \} \colon f(x) \geq f(a)$.
  \end{enumerate}
\end{definition}

\begin{definition}
  Nechť $f$ je funkce. Funkce $f$ se nazývá \textbf{periodická}, pokud $\exists p \in \mathbb R^{+}\colon \forall x \in \mathcal D(f)\colon$
  \begin{enumerate}
    \item $x \in \mathcal D(f) \implies x \pm p \in \mathcal D(f)$,
    \item $f(x) = f(x \pm p)$.
  \end{enumerate}
...
\begin{definition}
  Nechť máme funkci $f\colon y = f(u)$ s definičním oborem $\mathcal D(f)$ a funkci $g\colon u=g(x)$ s oborem hodnot $\mathcal H(g)$. Jestliže je $\mathcal H(g) \subseteq \mathcal D(f)$, pak funkci $h\colon y = f(g(x))$ nazveme \textbf{složenou funkcí} (někdy píšeme též $h=f \circ g$).
\end{definition}
...
\begin{definition}
  Nechť $x \in \mathbb{R}$ je libovolné číslo. Pak existuje právě jedna dvojice $z \in \mathbb{Z}, a \in \left<  0;1 \right) \text{ tak, že } x = z + a$.
Číslo $z$ nazýváme \textbf{celou částí} čísla $x$ a zapisujeme $[x] = z$, někdy také $\lfloor x\rfloor = z$.
\end{definition}
