...
\begin{definition}
    Nechť $k$ o rovnici \ref{kuzelosec} je kuželosečka. Rovnici každé kuželosečky lze
    vhodnou transformací soustavy souřadnic převést na jeden z následujících tvarů:
    \begin{enumerate}[$i.$]
    ...
    \end{enumerate}
   Tyto tvary nazýváme \textbf{kanonickými rovnicemi kuželoseček}.
\end{definition}
...
\begin{reseni}
Doplníme na čtverec. Dostaneme
$$
    (2x-4)^2 + (3y+9)^2 - 36 = 0.
$$
Po vydělení rovnice 36 dostaneme tvar
$$
\frac{(x-2)^2}{9} + \frac{(y+3)^2}{4} = 1,
$$
jedná se tak o elipsu.
\end{reseni}
...
Doplníme na čtverec. Dostaneme
$$
    5(x+2)^2 + 3(y - 4)^2-5\cdot 4 - 3 \cdot 16 + 38 = 0.
$$
Po úpravě máme
$$
    5(x+2)^2 + 3(y - 4)^2 = 30.
$$
Vydělíme 30 a dostaneme
$$
\frac{(x+2)^2}{6}+ \frac{(y-4)^2}{5} = 1.
$$
\end{reseni}
...
\begin{priklad}
Je dána hyperbola $k\colon 9x^2-16y^2-36x+32y-124=0.$ Najděte její středovou rovnici,
střed, poloosy, ohniska, vrcholy, rovnice asymptot a hyperbolu načrtněte.
\end{priklad}

\begin{reseni}
Středovou rovnici najdeme doplněním na čtverec. Dostaneme
$$
\frac{(x-2)^2}{8} - \frac{(y+1)^2}{\frac{2}{9}} = 1.
$$
Střed je tedy v bodě $[2,-1]$, poloosy mají délky $2 \sqrt{2}$ a $\frac{\sqrt{2}}{3}$. Excentricita je
$\sqrt{a^2 + b^2} = \sqrt{8 + \frac{2}{9}} = \sqrt{\frac{10}{9}} = \frac{\sqrt{10}}{3}$, takže
ohniska jsou v bodech $[2\pm \frac{\sqrt{10}}{3}, -1]$. Vrcholy jsou v bodech $[2 \pm \sqrt{8}, -1]$.
Asymptoty jsou přímky $[2, -1] + (\sqrt{8}, \frac{\sqrt{2}}{3})$ a $[2, -1] + (\sqrt{8}, -\frac{\sqrt{2}}{3})$.
\end{reseni}
...
\begin{priklad}
Určete vrchol, ohnisko, parametr a řídící přímku paraboly $k\colon y^2-7x-6y-19=0.$
\end{priklad}

\begin{reseni}
Doplníme na čtverec. Máme
$$
    (y-3)^2 - 9-19-7x=0,
$$
po převedení do tvaru rovnice paraboly dostaneme
$$
    (y-3)^2 = 2 \cdot \frac{7}{2}\cdot  x + 28.
$$
Vrchol je v bodě $[28,3]$. Ohnisko je pak v bodě $[28 + \frac{7}{4}, 3]$, parametr je $\frac{7}{2}$ a řídící přímka
je $x = 28 - \frac{7}{4}$.
\end{reseni}
...
\begin{veta}[Rovnice tečen regulárních kuželoseček v základní poloze]
    ...
    má rovnici
    \begin{enumerate}[$i.$]
    \item kružnice se středem v $S[m,n]\colon (x-m)(x_r-m)+(y-n)(y_r-n)=r^2,$
   	\item elipsa se středem v $S[m,n]\colon \frac{(x-m)(x_r-m)}{a^2}+\frac{(y-n)(y_r-n)}{b^2}=1$,
   	\item hyperbola se středem $S[m,n]\colon \frac{(x-m)(x_r-m)}{a^2}-\frac{(y-n)(y_r-n)}{b^2}=1$,
   	\item parabola s vrcholem $V[m,n]\colon (y-n)(y_r-n)=p(x+x_r-2m).$
    \end{enumerate}
\end{veta}
...
\begin{reseni}
Hledáme směr $\vec u=(u_1,u_2).$ Po dosazení do definice dostáváme podmínku $u_1^2 + 4u_1u_2 + a_2^2 = 0$.
Jelikož nám jde jen o směr vektoru, můžeme jenu z proměnných zvolit. Položme $u_1=1$. Dostáváme rovnici
$1+4u_2+u_2^2 = 0$, která má řešení $-2 \pm \sqrt{3}$. Hledané směry tedy jsou
$(1, -2 + \sqrt{3})$ a $(1, -2 -\sqrt{3})$.
\end{reseni}

\begin{definition}
    Bod $S$ je středem kuželosečky $k$, pokud $\forall X\in k\colon \exists X^\prime\in k\colon  S$ je středem $XX^\prime.$
\end{definition}
...
\end{pmatrix}=\vec o,
$$
je to singulární bod. V našem případě je
$$
    A = \begin{pmatrix}
	1 & 0 & 1\\
	0 & 1 & 0 \\
...
\end{pmatrix}.
$$
Střed kuželosečky je tedy bod $[0,0]$. Rovnice pro singulární bod by byla
$$
\begin{pmatrix}
	1 & 0 & 1\\
	0 & 1 & 0 \\
...
\end{pmatrix} = \vec o,
$$
jenže tato rovnice splněna není, a proto $[0,0]$ není singulární bod této kuželosečky.
\end{reseni}

\begin{priklad}
Je dána kuželosečka $y^2-xy-5x+7y+10=0.$ Určete
\begin{enumerate}[$a.$]
\item regularitu/singularitu,
\item asymptotické směry,
\item středy,
\item singulární body,
