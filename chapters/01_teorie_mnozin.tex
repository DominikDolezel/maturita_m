...
\begin{definition}
  ...
  Zapisujeme $B \subseteq A$.
\end{definition}

\begin{pozn}
Každá neprázdná množina má dvě \textbf{nevlastní podmnožiny} -- množinu prázdnou a sebe sama.
Všechny ostatní její podmnožiny nazýváme \textbf{vlastní}.

\end{pozn}
\begin{definition}
  Množiny $A$ a $B$ \textbf{se rovnají}, pokud $A$ je podmnožinou $B$ a zároveň $B$ je podmnožinou $A$.
\end{definition}

\begin{definition}
  Nechť $A \subseteq B$ a $B\neq \emptyset$. Množinu všech prvků množiny $B$, které nepatří do množiny $A$, nazýváme \textbf{doplněk} (komplement) množiny $A$ v množině $B$. Zapisujeme $A_B^\prime.$
\end{definition}
...
\begin{figure}[ht!]
  ...
  \begin{tikzpicture}[
      scale=1.2,
      set line/.style={draw=black, rounded corners=2.5mm, semithick},
      box/.style={draw=black, semithick}
  ]

  % Vlevo nahoře (2 množiny)
  \begin{scope}[shift={(0, 3)}]
      \draw[box] (0,0) rectangle (4,2.5);
      \node[anchor=north west, inner sep=3pt] at (0,2.5) {$U$};

      % Nahrazeno z (1,0) |- (4,0.9)
      \draw[set line] (1,0) -- (1,0.9) -- (4,0.9);
      \node at (0.7, 0.9) {$A$};

      % Nahrazeno z (2.8,0) |- (4,2.1)
      \draw[set line] (2.8,0) -- (2.8,2.1) -- (4,2.1);
      \node at (2.5, 2.1) {$B$};
  \end{scope}

  % Vpravo nahoře (3 množiny)
  \begin{scope}[shift={(4.5, 3)}]
      ...
      \draw[set line] (2.3,0) -- (2.3,1.7) -- (3.2,1.7) -- (3.2,0);
      \node at (2.0, 1.7) {$B$};
  ...
  \begin{scope}[shift={(0, 0)}]
      ...
      \draw[set line] (4,0.4) -- (1.5,0.4) -- (1.5,1.4) -- (4,1.4);
      \node at (1.2, 1.4) {$B$};

      \draw[set line] (2.5,0) -- (2.5,1.8) -- (3.3,1.8) -- (3.3,0);
      \node at (2.2, 1.8) {$C$};

      \draw[set line] (3.0,0) -- (3.0,2.2) -- (4,2.2);
      \node at (2.7, 2.2) {$D$};
  \end{scope}

  % Vpravo dole (4 množiny - alternativní zobrazení)
  \begin{scope}[shift={(4.5, 0)}]
      ...
      \draw[set line] (1.5,0.3) rectangle (3.7,1.3);
      \node at (1.2, 1.3) {$B$};

      \draw[set line] (2.1,0.1) rectangle (3.2,1.7);
      \node at (1.8, 1.7) {$C$};
  ...
  \end{tikzpicture}
  \caption{Vennovy diagramy pro dvě, tři a čtyři množiny}
\end{figure}
...
\begin{proof}
  ...
  \begin{tikzpicture}[
      scale=0.9,
      set/.style={thick, draw=black},
      univ/.style={thick, draw=black},
      fillcolor/.style={fill=green!50},
      hatchcolor/.style={pattern=north west lines, pattern color=gray!80},
      altfillcolor/.style={fill=yellow!30}
  ]

  \def\universe{(-2.5,-1.5) rectangle (2.5,1.5)}
  \def\setA{(-0.7,0) circle (1.1)}
  \def\setB{(0.7,0) circle (1.1)}

  % 1. Vlevo nahoře: A \cup B
  \begin{scope}[shift={(0,0)}]
      \fill[fillcolor] \setA;
      \fill[fillcolor] \setB;
      \draw[univ] \universe;
      \node[above left] at (2.5,-1.5) {$U$};
      \draw[set] \setA;
      \node[above left] at (-1.3,0.7) {$A$};
      \draw[set] \setB;
      \node[above right] at (1.3,0.7) {$B$};
      \node at (0,-2) {$(A \cup B)$};
  \end{scope}

  % 2. Vlevo dole: (A \cup B)'
  \begin{scope}[shift={(0,-4.5)}]
      \fill[fillcolor] \universe;
      \fill[white] \setA;
      \fill[white] \setB;
      \draw[univ] \universe;
      \node[above left] at (2.5,-1.5) {$U$};
  ...
  % 3. Vpravo nahoře: Zobrazení A' a B'
  \begin{scope}[shift={(6,0)}]
      % A' je žlutě (vše mimo A)
      \fill[altfillcolor, even odd rule] \setA \universe;
      % B' je šedě šrafovaně (vše mimo B)
      \fill[hatchcolor, even odd rule] \setB \universe;
...
\begin{pozn}
  Čísla zapisujeme pomocí dekadického pozičního systému, a to jako posloupnost číslic 0--9 (popř. desetinné čárky) a chápeme je jako sumu násobků mocnin desítky, přičemž násobek odpovídá číslici a mocnina odpovídá pořadí číslice v číslu, např.
  $$4503{,}6=4\cdot 10^3+5\cdot 10^2 + 0 \cdot 10^1 + 3\cdot 10^0 + 6 \cdot 10^{-1}.$$

  Číslice před desetinnou čárkou nazýváme celou částí, číslice po ní nazýváme desetinným rozvojem. Každé racionální číslo je v desítkové soutavě vyjádřeno buď ukončeným desetinným rozvojem nebo periodickým rozvojem. Iracionální číslo je vyjádřeno neukončeným neperiodickým rozvojem.
\end{pozn}

\begin{definition}
  \textbf{Reálnými čísly} nazýváme všechna čísla, která jsou délkami úseček.
\end{definition}

\begin{definition}
  Nechť $a,b \in \mathbb{R},$ kde $a<b$. Pak množiny takových $x\in \mathbb{R},$ že $a\leq x\leq b$
  (resp. $a < x < b$, resp. $a < x$, resp. $a \leq x < b$ atd.) nazýváme uzavřeným
  (resp. otevřeným, resp. neomezeným zleva otevřeným, resp. zprava uzavřeným, zleva otevřeným atd.) \textbf{intervalem}. Zapisujeme $\left< a,b\right>$ (resp. $\left(a,b\right)$, resp. $(a, \infty)$, resp. $\left< a, b\right)$ atd.).
\end{definition}

\begin{definition}
  \textbf{Periodický rozvoj čísla} je desetinný rozvoj, u kterého se za desetinnou čárkou donekonečna opakuje (periodicky opakuje) tatáž číslice nebo skupina číslic. Opakující se číslice nebo skupina opakujících se číslic se nazývá perioda. Zapisují se tak, že se nad opakující se skupinou napíše pruh.
  $$0{,}333\hdots = 0{,}\overline{3}.$$
\end{definition}
...
\begin{example}[SÚM 173/37*]
  Je dán trojúhelník $ABC$. Uvažujme množinu $M$ všech bodů tohoto trojúhelníku, pro které platí $|AX| \geq |BX| \geq |CX|.$ Pomocí velikosti stran a úhlů troj. $ABC$ vyjádřete podmínky pro to, aby:
  \begin{enumerate}[a.]
