...
integrál} z funkce $f(x)$ a značí se $\int f(x)\, \d x.$ Tedy
$$\int f(x)\, \d x= \left \{ F(x); F(x) \textrm{ je primitivní k } f(x) \textrm{ na } I \right \}. $$
\end{definition}
...
\begin{veta}
    Integrály tvaru
    $$\int S(x, \sqrt[s_1]{x}, \dots, \sqrt[s_k]{x}  ) \, \d x ,\quad \textrm{kde } k \in \mathbb N, s_i \geq 2$$
