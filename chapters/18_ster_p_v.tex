...
\begin{axiom}[Axiom rovnoběžnosti]
...
\begin{definition}
    Přímku $a \subseteq \mathbb E_3$ nazveme s rovinou $\alpha \subseteq \mathbb E_3\colon$
    \begin{enumerate}[$i.$]
...
\begin{definition}\label{kolmeprimky}
    Přímky $p,q \subseteq \mathbb E_3$ se nazývají navzájem \textbf{kolmé} právě tehdy, když
...
\begin{definition}
    ...
    když $\forall q \subseteq \alpha\colon q \perp p.$
\end{definition}

\begin{veta}[Kritérium kolmosti přímky a roviny]
    Nechť $p\subseteq \mathbb E_3$ je přímka a $\alpha \subseteq \mathbb E_3$ je rovina.
    Pak  $p\perp \alpha \iff \exists q,r \subseteq \alpha\colon q \nparallel r\colon q\perp p\land
    r\perp p.$
\end{veta}
...
\begin{definition}\label{kolmeroviny}
    ...
    $\exists a\subseteq\alpha\colon a \perp\beta.$
\end{definition}
