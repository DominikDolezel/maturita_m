...
\begin{definition}
Nechť $A$ je množina. Zobrazení $a\colon\mathbb N\to A$ (nebo $a\colon\left \{ 1,2,...,k \right \} \to A$)
...
\begin{enumerate}[$i.$]
\item \textbf{rostoucí}, pokud $\forall n \in \mathbb N\colon a_n< a_{n+1},$
\item \textbf{klesající}, pokud $\forall n \in \mathbb N\colon a_n> a_{n+1},$
\item \textbf{nerostoucí}, pokud $\forall n \in \mathbb N\colon a_n\geq a_{n+1},$
\item \textbf{neklesající}, pokud $\forall n \in \mathbb N\colon a_n\leq a_{n+1}.$
\end{enumerate}
Posloupnost, která má nějakou z těchto vlastností, se nazývá \textbf{monotónní}.
Posloupnost rostoucí nebo klesající se nazývá \textbf{ryze monotónní}.
\end{definition}
...
\begin{reseni}
Spočítáme $a_{n+1}-a_n$. Je
$$
    a_{n+1}-a_n = \frac{2(n+1)}{n+2} - \frac{2n}{n+1} = \frac{2}{(n+2)(n+1)},
$$
což je kladné číslo pro všechna $n \in \mathbb N $, a tedy posloupnost je rostoucí.
\end{reseni}

\begin{definition}
    Nechť $\left \{ a_n \right \}_{n=1}^\infty $ je posloupnost reálných čísel. Pak je
    \begin{enumerate}[$i.$]
    \item \textbf{prostá}, pokud $\forall m,n\in \mathbb N\colon m\ne n \implies a_m\ne a_n,$
   	\item \textbf{stacionární} (konstantní), pokud $\forall n \in \mathbb N\colon a_n = a_{n+1}.$
    \end{enumerate}
\end{definition}
...
\begin{definition}
    ...
    $\forall n \in \mathbb N\colon a_n \geq k,$
   	\item \textbf{shora omezená}, pokud existuje $k\in \mathbb R$ takové, že
    $\forall n \in \mathbb N\colon a_n \leq k.$
    \end{enumerate}
...
\begin{definition}
Nechť $\left \{ a_n \right \}_{n=1}^\infty $ je posloupnost a
$\left \{ k_1,k_2,\dots,k_n,\dots \right \} $ je rostoucí posloupnost přirozených čísel.
Pak posloupnost $\left \{a_{k_n} \right \}_{n=1}^\infty = \left \{ a_{k_1}, a_{k_2}, \dots, a_{k_n},\dots \right \}  $
je \textbf{posloupnost vybraná z posloupnosti} $\left \{ a_n \right \}_{n=1}^\infty $.
\end{definition}

\begin{definition}
Posloupnost $\left \{ a_n \right \}_{n=1}^\infty $ je \textbf{aritmetická}, pokud
$$\exists d \in \mathbb R\colon \forall n \in \mathbb N\colon a_n=a_{n-1}+d,$$
číslo $d$ se nazývá \textbf{diference}.
\end{definition}
...
\begin{veta}
    ...
    $d$. Pak platí
    \begin{enumerate}[$i.$]
    \item $\forall n\in \mathbb N\colon a_n = a_1 + (n-1)d,$
   	\item $\forall r,s \in \mathbb N\colon a_s = a_r + (s-r)d.$
    \end{enumerate}
\end{veta}

\begin{proof} Dokazujeme postupně.
\begin{enumerate}[$i.$]
\item Matematickou indukcí:
\begin{enumerate}[1.]
\item $n=1\colon a_1 = a_1+(1-1)d$ -- platí
\item $a_n = a_1+(n-1)d \implies a_{n+1}=a_1+nd,$ takže $a_{n+1}=a_n+d=a_1+(n-1)d+d=a_1+nd$
\end{enumerate}
\item Užitím již dokázeného vztahu $i.$:
\begin{align*}
    a_s &= a_1 + (s-1)d,\\
    a_r &= a_1 + (r-1)d.
\end{align*}
...
$$\forall n \in \mathbb N\colon S_n = \frac{1}{2}n(a_1+a_n).$$
\end{veta}
...
\begin{reseni}
Uvažujme aritmetickou posloupnost s prvním členem $a_1$ a diferencí $d$. Pak je $a_4 = a_1 + 3d$,
$a_2 = a_1 + d$ a $a_6 = a_1 + 5d$. Soustavu tedy můžeme přespat jako
\begin{align*}
	a_1 + a_1 + 3d &= 12, \\
	a_1 + d - a_1 - 5d &= -8.
\end{align*}
...
\begin{align*}
	...
	- 4d  &= -8,
\end{align*}
odkud vidíme, že $d= 2$. Dosazením do první rovnice pak dostaneme $a_1=3$. Hledaná posloupnost tedy začíná
$$
...
\begin{definition}
Posloupnost $\left \{ a_n \right \}_{n=1}^\infty $ je \textbf{geometrická}, pokud
$$\exists q \in \mathbb R\colon \forall n \in \mathbb N\colon a_n=a_{n-1}\cdot q,$$
číslo $q$ se nazývá \textbf{kvocient}.
\end{definition}
...
\begin{reseni}
Je-li posloupnost aritmetická, platí $a_{n+1} = a_n + d$. Ověříme tuto podmínku. Je
$$
    a_{n+1} = \frac{2(n+1)-1}{3} = \frac{2n+1}{3},
$$
tedy
$$
    a_{n+1}-a_n = \frac{2n+1}{3}-\frac{2n-1}{3}=\frac{2}{3},
$$
a proto je posloupnost $a_n$ aritmetická s diferencí $\frac{2}{3}$. To, jestli je geometrická, bychom ověřovali podobně,
počítali bychom $\frac{a_{n+1}}{a_n}$.
\end{reseni}
...
\begin{veta}
    ...
    $q$. Pak platí
    \begin{enumerate}[$i.$]
    \item $\forall n\in \mathbb N\colon a_n = a_1 \cdot q^{n-1},$
   	\item $\forall r,s \in \mathbb N\colon a_s = a_r \cdot q^{s-r}.$
    \end{enumerate}
\end{veta}
...
\item Matematickou indukcí:
\begin{enumerate}[1.]
\item $n=1\colon a_1 = a_1\cdot q^0$ platí
...
$n$-členné geometrické posloupnosti. Pak $\forall n \in \mathbb N$ platí
\begin{enumerate}[$i.$]
\item $q=1\colon S_n=na_1,$
\item $q\ne 1\colon S_n = a_1\cdot \frac{q^n-1}{q-1}.$
\end{enumerate}
\end{veta}
...
\begin{reseni}
Uvažujme geometrickou posloupnost s prvním prvkem $a_1$ a kvocientem $q$. Pak je $a_3 = a_1 q^2$, $a_2 = a_1q$ a
$a_4 = a_1 q^3$. Soustavu můžeme přepsat jako
\begin{align*}
	a_1 + a_1q^2 &= 5, \\
	a_1q + a_1q^3 &= 10.
\end{align*}
Vidíme, že druhá rovnice je $q$-násobkem první rovnice, a tedy $q=2$. Dosazením do první rovnice dostaneme
$$
    a_1 + 4a_1 = 5a_1 = 5,
$$
tedy $a_1 = 1$. Hledaná posloupnost tedy začíná prvky
$$
...
\begin{priklad}
Nalezněte explicitní vyjádření posloupnosti zadané rekurentně: $$a_{n+1}=3a_n+1, \quad a_1=2.$$
\end{priklad}

\begin{reseni}
Napišme si prvních pár členů této posloupnosti:
\begin{align*}
	a_1 &= 2, \\
	a_2 &= 3 \cdot 2 + 1, \\
	a_3 &= 3(3\cdot 2 + 1)+1 = 2 \cdot 3^2 + 3+1, \\
	a_4 &= 3(3 (3\cdot 2 + 1)+1)+1 = 2\cdot 3^3 + 3^2+3+1.
\end{align*}
Odsud usoudíme, že $a_n = 2\cdot 3^{n-1}+\sum_{i=0}^{n-2}3^i$. Tuto hypotézu nyní dokážeme matematickou indukcí.
\begin{itemize}
	\item Pro $n = 1$ vzorec platí.
	\item Předpokládejme, že pro $n$ vzorec platí. Ukážeme, že platí i pro $n+1$. Prvek $a_{n+1}$ je
	$$
	    3\cdot \left( 2\cdot 3^{n-1}+\sum_{i=0}^{n-2}3^i \right ) +1 = 2\cdot 3^n + \sum_{i=1}^{n-1} 3^i + 1 = 2\cdot 3^n + \sum_{i=0}^{n-1} 3^i,
	$$
	tedy vzorec platí i pro $n+1$. Tím je důkaz u konce.
\end{itemize}
\end{reseni}
...
$A,$ jestliže
$$\forall \varepsilon \in \mathbb R^+\colon \exists n_0 \in \mathbb N_0\colon \forall n \geq n_0,n \in \mathbb N\colon a_n\in(A-\varepsilon, A+\varepsilon).$$
\end{definition}
...
Počítejme
$$
    \lim_{n\to\infty}\frac{n+1}{n} = \lim_{n \to \infty} \frac{n+1}{n} \cdot \frac{\frac{1}{n }}{\frac{1}{n}} = \lim_{n \to \infty} \frac{1+\frac{1}{n }}{1},
$$
ale $\lim_{n \to \infty} \frac{1}{n} = 0$, a tedy podle věty o aritmetice limit je limita ze zadání rovna 1.
\end{reseni}
...
$$\forall K \in \mathbb R\colon \exists n_0 \in \mathbb N_0\colon \forall n \geq n_0, n\in \mathbb N\colon a_n>K \,\, (\textrm{resp. } a_n<K).$$
\end{definition}
...
Počítejme
$$
\lim_{n \to \infty} \frac{4-n^2}{2+n} = \lim_{n \to \infty} \frac{(2-n)(2+n)}{2+n} = \lim_{n \to \infty} 2-n = -\infty.
$$
\end{reseni}
...
\begin{veta}[Věta o třech limitách]
    ...
    $$\exists m\in \mathbb N\colon \forall n \geq m\colon a_n \leq b_n \leq c_n \land \lim_{n\to \infty}a_n=\lim_{n\to \infty}c_n=A.$$
    Pak $\lim_{n\to\infty}b_n=A.$
\end{veta}

\begin{veta}
Nechť jsou dány dvě posloupnosti $\left \{ a_n \right \}_{n=1}^\infty $, $\left \{ b_n \right \}_{n=1}^\infty$. Nechť $\lim_{n\to \infty}a_n = A$, $\lim_{n\to \infty}b_n = B$
...
nazveme \textbf{horní} (resp. \textbf{dolní}) \textbf{závorou} množiny $M$, pokud
$$\forall x \in M\colon x\leq a \,\, (\textrm{resp. }x\geq a).$$
\end{definition}
...
\begin{enumerate}[$i.$]
\item $\forall x\in M\colon x\leq c \,\, (\textrm{resp. } x\geq c),$
\item $\forall t\in \mathbb R\colon \forall x\in  M\colon x\leq t \implies t\geq c \,\, (\textrm{resp. }x\geq t \implies t\leq c).$
\end{enumerate}
...
$$S_n=a_1+a_2+\dots+a_n = \sum_{i=1}^n a_i,\quad n\in \mathbb N$$
nazýváme \textbf{$n$-tým částečným součtem} posloupnosti $\left \{ a_n \right \}_{n=1}^\infty $. Výraz
$$a_1+a_2+a_3+\dots$$
nazveme \textbf{nekonečnou řadou} a značíme stručně
$$\sum_{n=1}^\infty a_n.$$
...
\begin{veta}[Srovnávací kritérium konvergence]
Nechť $\sum_{n=1}^\infty a_n, \sum_{n=1}^\infty b_n$ jsou řady s nezápornými členy
s vlastností
$$\forall n \in \mathbb N\colon a_n \leq b_n.$$
Pak platí
\begin{enumerate}[$i.$]
\item konverguje-li řada $\sum_{n=1}^\infty b_n,$ konverguje také $\sum_{n=1}^\infty a_n.$
\item diverguje-li řada $\sum_{n=1}^\infty a_n,$ diverguje také $\sum_{n=1}^\infty b_n.$
\end{enumerate}
\end{veta}
...
$$
\lim_{n\to\infty}\frac{\frac{n^2+n+1}{n^3+1}}{\frac{1}{n}} = \lim_{n \to \infty}{ \frac{n^3 +n^2+n }{n^3+1}} = \lim_{n\to \infty}{\frac{1+\frac{1}{n}+\frac{1}{n^2}}{1+\frac{1}{n^2}}} = 1,
$$
tedy podle limitního srovnávacího kritéria řada $\sum_{n=1}^\infty \frac{n^2+n+1}{n^3+1}$ také diverguje.
\end{reseni}
...
\begin{reseni}
	Označme délku strany prvního čtverce $a_1=a$, druhého $a_2$, \dots Z Pythagorovy věty máme, že
	$a_{n+1}=\sqrt{2} \frac{a_n}{2} = \frac{a_n}{\sqrt{2}}$. Součet obvodů těchto čtverců je tedy
	$\sum_{n=0} ^\infty \frac{a}{\left( \sqrt{2} \right )^{n-1} }$. To je geometrická řada s prvním členem
	$a$ a kvocientem $\left |\frac{1}{\sqrt{2}}\right | < 1$, můžeme tedy použít větu \ref{konvergencerad}.
	Pak je
	$$
	    O = \frac{a}{1- \frac{1}{\sqrt{2}}}.
	$$
	Obdobně součet obsahů je řada $\sum_{n=0}^\infty \left( \frac{a }{\sqrt{2}} \right )^2 $, což je geometrická
	řada s prvním prvkem $a^2$ koeficientem $\frac{1}{2}$. Součet je tedy podle věty \ref {konvergencerad}
	$$
