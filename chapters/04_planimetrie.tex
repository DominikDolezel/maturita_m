...
\begin{proof}
  Ukážeme, že $|AC| < |AB| + |BC|$. Sestrojme na $\overrightarrow{AB}$ bod $D$ tak, že $|BD| = |BC| \land B\mezi AD$. $\triangle BCD$ je rovnoramenný s hlavním vrcholem B $\implies \omega_1 = \delta$
  $B \mezi AD \implies \omega_1 < \omega$
...
\begin{veta}\label{mbdv}
  Nechť je dána úsečka $AB$. Množinou všech bodů $X\in \mathbb E_2,$ pro něž $|AX|=|BX|$, je \textbf{osa} $o$ \textbf{úsečky} $AB$.
\end{veta}

\begin{proof}
  Označme $U=\{ X\in \mathbb E_2\colon |AX|=|BX| \}$.
  \begin{enumerate}[$i.$]
...
\begin{veta}
  ...
  \begin{enumerate}[$i.$]
    \item $\overrightarrow{oA}=\{ X\in \mathbb E_2\colon|AX|\leq |BX| \},$
    \item $\overrightarrow{oB}=\{ X \in \mathbb E_2 \colon |AX| \geq |BX| \}.$
  \end{enumerate}
\end{veta}
...
\begin{definition}
  Nechť je dána úsečka $AB$. Množinou všech vrcholů $C\in \mathbb E_2$ všech pravoúhlých trojúhelníků $\triangle ABC$ s přeponou $AB$ je množina $k\smallsetminus\{ A,B\}$, kde $k\left (S,\frac{|AB|}{2}\right )$ je kružnice, $S$~je střed $AB$. Tuto množinu budeme nazývat \textbf{Thaletovou kružnicí} nad přeponou $AB$ a~označovat $\tau (AB).$
\end{definition}
