...
\begin{veta}[Bayesův vzorec]
    ...
    \emptyset.$ Nechť $\exists i \in \left \{ 1, 2, \dots, n \right \} \colon P(B_i) >0.$
    Pak platí: $\forall k\in \left \{ 1, 2, \dots, n \right \}\colon$
...
\begin{align*}
 P(D)\cdot P(M \mid D)&=P(M \cap D)=P(M)\cdot P(D \, | M)\\
 P(D \,|\, M) &= \frac{P(D)\cdot P(M \mid D)}{P(M)}=\frac{P(D)\cdot P(M \mid D)}{P(D)\cdot P(M\mid D)+P(H)\cdot P(M \mid H)}=\frac{3}{8}.
\end{align*}
\end{reseni}
