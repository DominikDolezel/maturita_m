...
\begin{priklad}
Znázorněte graf relace $U=\left \{ \left [ x,y \right ]\in \mathbb R^2\colon x-y+1=0  \right \}. $
\end{priklad}

\begin{reseni}
Řešením rovnice $x-y+1=0$ dostaneme tvar $y=x+1$. Graf je znázorněn v obrázku \ref{grafx+1}.
\end{reseni}
...
\begin{figure}[ht!]
    ...
    \end{axis}
    \end{tikzpicture}
	\caption{Graf relace $U=\left \{ \left [ x,y \right ]\in \mathbb R^2\colon x-y+1=0  \right \} $}
	\label{grafx+1}
\end{figure}
...
\begin{definition}\label{reflsymtran}
  ...
  \begin{enumerate}[$i.$]
    \item \textbf{reflexivní}, pokud $\forall a\in A\colon a\sim a$,
    \item \textbf{symetrickou}, pokud $\forall a,b \in A\colon a\sim b \implies b\sim a,$
    \item \textbf{tranzitivní}, pokud $\forall a,b,c\in A\colon a\sim b \land b\sim c \implies a\sim c.$
  \end{enumerate}
\end{definition}

\begin{definition}\label{ekvivalence}
...
\begin{definition}
  Nechť $A,B$ jsou dvě množiny. \textbf{Zobrazením} $f$ \textbf{z množiny $A$ do množiny $B$} nazýváme relaci $f\subseteq A \times B,$ pro níž platí: $\forall x \in A\colon \exists \text{ max. 1 } y \in B\colon (x,y) \in f$. Prvek $x$ nazveme \textbf{vzorem} a prvek $y$ \textbf{obrazem}.
\end{definition}

\begin{definition}
  Nechť $f\subseteq A\times B$ je zobrazení. \textbf{Definičním oborem} (resp. \textbf{oborem hodnot}) zobrazení $f$ nazveme množinu $\mathcal D(f)\subseteq A$
  (resp. $\mathcal H(f)\subseteq B$) všech prvků $a\in A$ (resp. $b\in B$) takových, že k nim existuje právě jedno $b\in B$ (resp. alespoň jedno $a\in A$) tak, že $b=f(a)$.
\end{definition}

\begin{definition}
  Pokud $\mathcal D(f) = A$ (resp. $\mathcal D(f)\ne A$), hovoříme o \textbf{zobrazení množiny} (resp. \textbf{zobrazení z množiny}) $A$. Pokud $\mathcal H(f)=B$ (resp. $\mathcal H(f)\ne B$), hovoříme o \textbf{zobrazení na množinu} (resp. \textbf{zobrazení do množiny}) $B$.
  Zobrazení množiny na množinu nazýváme \textbf{surjekcí}. Je-li $A=B$ (resp. $A=B \land ( A = \mathcal D(f) \lor B = \mathcal H(f))$), hovoříme o \textbf{zobrazení v množině} (resp. \textbf{zobrazení na množině}) $A$.
\end{definition}
...
\begin{definition}
  ...
  \]
  Jsou-li $\alpha$ i $\alpha^{-1}$ zobrazení, nazveme relaci $\alpha ^{-1}$ \textbf{inverzním zobrazením} k zobrazení $\alpha$.
\end{definition}

\begin{definition}
  Nechť $f\colon B\to C, g\colon A\to B$ jsou zobrazení. \textbf{Složeným zobrazením} ze zobrazení $f$ a $g$ nazveme zobrazení $h\colon A\to C$ takové, že
  \[
    h=\left\{ (a,c)\in A\times C;\exists b\in B\colon f(b)=c \land g(a)=b \right\}.
  \]
