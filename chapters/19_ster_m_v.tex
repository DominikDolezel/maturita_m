...
\begin{definition}
    Nechť $A, B \in \mathbb E_3.$ \textbf{Vzdáleností bodů} $A,B$ nazýváme délku
...
\begin{definition}
    ...
    p,q|$ a definované tak, že
    \begin{enumerate}[$i.$]
    \item je-li $p\parallel q, |\sphericalangle p,q|=0^\circ,$
   	\item je-li $p\nparallel q$, $|\sphericalangle p,q|$ se rovná velikosti ostrého nebo pravého úhlu,
        který $p,q$ svírají.
    \end{enumerate}
\end{definition}
...
\begin{reseni}
Protože jsou přímky $CD$ a $AB$ rovnoběžné, stačí počítat odchylku přímek $\overleftrightarrow{AV}$
...
\begin{figure}[h]
	...
	\centering
	\tikzfig{jehlan}
	\caption{Pravidelný čtyřboký jehlan $ABCDV$}
	\end{subfigure}
	\begin{subfigure}{.5\textwidth}
	\centering
	\tikzfig{rez_trojuhelnik}
	\caption{Trojúhelník $ABV$}
	\end{subfigure}


	\caption{Řešení příkladu \ref{odchpr}}
\end{figure}
\end{reseni}
...
\begin{definition}
    ...
    jsou komplanární (a nutně různoběžné).
\end{definition}
...
\begin{definition}
    ...
    \item je-li $p\nparallel \alpha$, $|\sphericalangle p,\alpha|$ se rovná velikosti ostrého nebo
        pravého úhlu,
...
\begin{pozn}
	Čtyřstěn je nejjednodušší příklad mnohostěnu.
\end{pozn}

\begin{definition}
\textbf{Těleso} je uzavřená část prostoru. \textbf{Mnohostěn} je uzavřená část prostoru, jejiž hranice je tvořena konečným počtem
mnohoúhelníků, tyto mnohoúelníky nazveme \textbf{stěnami} mnohostěnu, úsečky, ve kterých se potkávají dvě stěny nazveme \textbf{hranami} a body, kde se potkávají alespoň tři hrany nazveme \textbf{vrcholy}. \textbf{Pravidelný mnohostěn} je mnohostěn, jehož všechny stěny jsou identické pravidelné mnohoúhelníky.
\end{definition}
...
\begin{definition}
    ...
    $\rho$ zobrazí na rovinu $\rho^\prime,$ tyto dvě roviny jsou rovnoběžné. Množinu všech bodů všech úseček $BB^\prime$ takových, že $B\in A_1\dots A_n$ a $B^\prime$ je
...
\begin{definition}
Mějme v prostoru rovinu $\rho,$ v ní konvexní mnohoúhelník $A_1\dots A_n$ a nechť
$V$ je bod, který v rovině $\rho$ neleží. Množinu všech bodů včech úseček $VX$, kde $X$ jsou všechny body
...
\begin{definition}
    ...
    $\rho.$ Množina všech bodů všech úseček $VX,$ kde $X\in K,$ se nazývá \textbf{kužel}.
