\section{Stereometrie -- metrické vlastnosti}
\begin{definition}
    Nechť $A, B \in \mathbb E_3.$ \textbf{Vzdálenost bodů} $A,B$ nazýváme délku
    úsečky $AB$ a~označujeme $\varrho(A,B).$
\end{definition}

\begin{definition}
    Nechť $A\in \mathbb E_3$ je bod a $\alpha \subset \mathbb E_3$ je rovina.
    \textbf{Kolmým průmětem} bodu $A$ do roviny $\alpha$ nazýváme bod $A_0$
    splňující
    \begin{enumerate}[$i.$]
    \item $A\in \alpha \implies A_0=A,$
   	\item $A\notin \alpha \implies A_0 \in p\cap \alpha, p \perp\alpha, A \in p.$
    \end{enumerate}
    \textbf{Vzdáleností bodu $A$ od roviny $\alpha$} nazýváme reálné číslo označené
    $\varrho(A,\alpha) $ a definované
    $$\varrho(A,\alpha) = \varrho(A, A_0) = |AA_0|,$$
    kde $A_0$ je kolmý průmět bodu $A$ do roviny $\alpha.$
\end{definition}

\begin{priklad}\label{prkvadr}
Určete vzdálenost bodu $F$ od roviny $\overleftrightarrow{BEG}$ v pravidelném
čtyřbokém hranolu $ABCDEFGH$, kde $|AB|=|BC|=a, |AE|=b$.
\end{priklad}

\begin{reseni}
Musíme najít kolmici na rovinu $\overleftrightarrow{BEG}$, která prochází bodem $F$. Rovina $\overleftrightarrow{BDF}$
je kolmá na $\overleftrightarrow{BEG}$, neboť má kvádr podstavu čtverce. Řez touto rovinou je na obrázku
\ref{prrez}. Z Pythagorovy věty je $|BD| = \sqrt{2}a$ a $|BF|=b$. Bod $X$ leží uprostřed úsečky $FH$,
proto má úsečka $FX$ délku $\frac{\sqrt{2}a }{2}$. Dále víme, že úhel $\sphericalangle FXB$ má velikost
$\arctg \frac{b}{\frac{\sqrt{2}a }{2}}$. Nyní už ze sinové věty máme, že
$$
\frac{\frac{\sqrt{2}a }{2}}{\sin 90^\circ} = \frac{x}{\arctg \frac{2b}{\sqrt{2}a}}.
$$
Odtud máme, že vzdálenost bodu $F$ od roviny $\overleftrightarrow{BEG}$
$$
x= \frac{\sqrt{2}a }{2} \cdot \arctg \frac{2b}{\sqrt{2}a}.
$$
\begin{figure}[h]
\begin{subfigure}{.5\textwidth}
\centering
	\begin{tikzpicture}[scale=1, line cap=round, line join=round]
   % Definice vrcholů kvádru ABCDEFGH
   % Dolní podstava ABCD
   \coordinate (A) at (0,0);
   \coordinate (B) at (4,0);
   \coordinate (C) at (5.5,2);
   \coordinate (D) at (1.5,2);

   % Horní podstava EFGH (posunutá svisle nahoru)
   \coordinate (E) at (0,2.5);
   \coordinate (F) at (4,2.5);
   \coordinate (G) at (5.5,4.5);
   \coordinate (H) at (1.5,4.5);

   \coordinate (X) at (2.75,3.5);

   % Neviditelné hrany (čárkovaně)
   \draw[dashed] (D) -- (A);
   \draw[dashed] (D) -- (C);
   \draw[dashed] (D) -- (H);

   % Viditelné hrany dolní podstavy a svislé hrany
   \draw (A) -- (B);
   \draw (B) -- (C);
   \draw (A) -- (E);
   \draw (B) -- (F);
   \draw (C) -- (G);

   \draw[dashed] (H) -- (F);
   \draw[dashed] (B) -- (D);
   \draw[dashed, blue] (X) -- (B);

   % Horní podstava
   \draw (E) -- (F) -- (G) -- (H) -- cycle;

   % Zvýraznění roviny BEG (trojúhelník BEG) modrou barvou
   \draw[thick, blue] (B) -- (E) -- (G) -- cycle;

   % Popisky vrcholů
   \node[below left] at (A) {$A$};
   \node[below] at (B) {$B$};
   \node[right] at (C) {$C$};
   \node[above left] at (D) {$D$};
   \node[left] at (E) {$E$};
   \node[below right] at (F) {$F$};
   \node[above right] at (G) {$G$};
   \node[above left] at (H) {$H$};
   \node[above] at (X) {$X$};
	\end{tikzpicture}
	\caption{Kvádr $A$--$H$}
	\label{fig:label}
\end{subfigure}
\begin{subfigure}{.5\textwidth}
	\centering
	\begin{tikzpicture}[scale=1.5]
		\coordinate (D) at (0,0);
		\coordinate (B) at (3,0);
		\coordinate (F) at (3,1.5);
		\coordinate (H) at (0, 1.5);
		\coordinate (X) at (1.5,1.5);

		\draw (D) -- (B) -- (F) -- (H) -- cycle;
		\draw[dashed] (X) -- (B);

		\node[below left] at (D) {$D$};
		\node[below right] at (B) {$B$};
		\node[above right] at (F) {$F$};
		\node[above left] at (H) {$H$};
		\node[above] at (X) {$X$};
	\end{tikzpicture}
	\caption{Řez rovinou $\overleftrightarrow{BDF}$}
	\label{prrez}
\end{subfigure}
\caption{Řešení příkladu \ref{prkvadr}}
\end{figure}
\end{reseni}

\begin{definition}
    Nechť $A\in \mathbb E_3$ je bod a $p \subset \mathbb E_3$ je přímka.
    \textbf{Kolmým průmětem} bodu $A$ na přímku $p$ nazýváme bod $A_0$
    splňující
    \begin{enumerate}[$i.$]
    \item $A\in p \implies A_0=A,$
    \item $A\notin p \implies A_0 \in p \cap q, q \perp p, A \in q.$
    \end{enumerate}
    \textbf{Vzdáleností bodu $A$ od přímky $p$} nazýváme reálné číslo označené
    $\varrho(A,p) $ a definované
    $$\varrho(A,p) = \varrho(A, A_0) = |AA_0|,$$
    kde $A_0$ je kolmý průmět bodu $A$ na přímku $p.$
\end{definition}

\begin{definition}
    Nechť $\alpha, \beta \subset \mathbb E_3$ jsou dvě rovnoběžné roviny. Pak
    \textbf{vzdáleností dvou rovnoběžných rovin} $\alpha, \beta$ nazýváme reálné
    číslo označené $\varrho(\alpha, \beta)$ a definované
    $$\varrho(\alpha, \beta)=\varrho(A,\beta),$$
    kde $A\in\alpha$ je libovolný bod.
\end{definition}

\begin{definition}
    Nechť $p \subset \mathbb E_3$ je přímka, $\alpha \subset \mathbb E_3$ je rovina.
    Nechť $p\parallel \alpha.$ \textbf{Vzdáleností přímky $p$ od roviny $\alpha$
    s ní rovnoběžné} nazýváme reálné číslo označené $\varrho(p,\alpha)$ a definované
    $$\varrho(p,\alpha)=\varrho(A,\alpha),$$
    kde $A\in p$ je libovolný bod.
\end{definition}

\begin{definition}
    Nechť $p,q \subset \mathbb E_3$ jsou rovnoběžné přímky. \textbf{Vzdáleností
    dvou rovnoběžných přímek $p,q$} nazýváme reálné číslo označené $\varrho(p,q)$ a
    definované
    $$\varrho(p,q) = \varrho(A,q),$$
    kde $A\in p$ je libovolný bod.
\end{definition}

\begin{definition}
    Nechť $p,q \subset \mathbb E_3$ jsou mimoběžné přímky. \textbf{Vzdáleností dvou
    mimoběžných přímek} $p,q$ nazýváme reálné číslo označené $\varrho(p,q)$ a definované
    $$\varrho(p,q)=\varrho(\alpha, \beta),$$
    kde $\alpha \parallel \beta, p \subset \alpha, q \subset \beta.$
\end{definition}

\begin{definition}
    Nechť $p,q \subset \mathbb E_3$ jsou dvě komplanární přímky. \textbf{Odchylkou
    dvou komplanárních přímek} $p,q$ nazýváme reálné číslo označené $|\sphericalangle
    p,q|$ a definované
    \begin{enumerate}[$i.$]
    \item je-li $p\parallel q, |\sphericalangle p,q|=0^\circ,$
   	\item je-li $p\nparallel q$, odchylka $p,q$ je velikost ostrého nebo pravého úhlu,
        který $p,q$ svírají.
    \end{enumerate}
\end{definition}

\begin{veta}
    Nechť jsou dány přímky $p, q, p^\prime, q^\prime \subset \mathbb E_3$ takové, že
    $p\parallel p^\prime$ a $q\parallel q^\prime.$
    Pak úhel, který svírají přímky $p$ a $q$ je roven úhlu, který svírají přímky $p^\prime$ a $q^\prime$.
\end{veta}

\begin{priklad}
Je dán pravidelný čtyřboký jehlan $ABCDV$ s podstavnou hranou $a$ a výškou $v$.
Určete odchylku přímek $p,q$, kde $p=\overleftrightarrow{AV}, q=\overleftrightarrow{CD}.$
\end{priklad}

\begin{reseni}
Protože jsou přímky $CD$ a $AB$ rovnoběžné, stačí počítat odchylku přímek $\overleftrightarrow{AV}$ a
$\overleftrightarrow{AB}$.
Z Pythagorovy věty je délka úsečky $AC$ rovna $\sqrt{2}a$, a tedy délka úsečky $AS$ je $\frac{\sqrt{2}a }{2}$.
Délky úseček $AX$ a $SX$ jsou shodně $\frac{a }{2}$. Opět z Pythagorovy věty je délka úsečky $VX$
rovna
$$\sqrt{\left(\frac{a }{2}\right)^2 + v^2}.$$
Odchylka přímek $\overleftrightarrow{AV}$ a
$\overleftrightarrow{AB}$ je pak rovna velikosti úhlu $\sphericalangle VAX$, což je
$$
    \arctg \left( \frac{\sqrt{\left(\frac{a }{2}\right)^2 + v^2}}{\frac{a }{2}} \right ) .
$$
\begin{figure}[h]
	\begin{subfigure}{.5\textwidth}
	\centering
	\begin{tikzpicture}[tdplot_main_coords, scale=1.4]
		\coordinate (A) at (0, 0, 0);
		\coordinate (B) at (2, 0, 0);
		\coordinate (C) at (2,2,0);
		\coordinate (D) at (0, 2, 0);
		\coordinate (V) at (1,1, 3);
		\coordinate (S) at (1,1, 0);
		\coordinate (X) at (1,0,0);

		\draw[thick] (A) -- (B) -- (C);
		\draw[thick, dashed, blue] (C) -- (D);
	    \draw[thick, dashed] (D) -- (A);
		\draw[blue, thick] (A) -- (V);
		\draw[thick] (B) --(V);
		\draw[thick] (C) -- (V);
		\draw[thick] (D) -- (V);
		\draw[dashed] (S) -- (V);
		\draw[dashed] (A) -- (S);
		\draw[dashed] (S)--(X)--(V);

		\node[below left] at (A) {$A$};
		\node[below right] at (B) {$B$};
		\node[right] at (C) {$C$};
		\node[left] at (D) {$D$};
		\node[above] at (V) {$V$};
		\node[below right] at (S) {$S$};
		\node[below left] at (X) {$X$};
	\end{tikzpicture}
	\caption{Pravidelný čtyřboký hranol $ABCDV$}
	\end{subfigure}
	\begin{subfigure}{.5\textwidth}
	\centering
	\begin{tikzpicture}[scale=1.5]
		\coordinate (A) at (0,0);
		\coordinate (B) at (2, 0);
		\coordinate (V) at (1, 3);
		\coordinate (S) at (1,0);

		\draw [thick] (A)--(B)--(V) --cycle;
		\draw [dashed] (S)--(V);

		\node[below left] at (A) {$A$};
		\node[below right] at (B) {$B$};
		\node[above] at (V) {$V$};
		\node[below] at (S) {$X$};
	\end{tikzpicture}
	\caption{Trojúhelník $ABV$}
	\end{subfigure}


	\caption{Čtyřboký hranol}
\end{figure}
\end{reseni}

\begin{definition}
    Nechť $p,q \subset \mathbb E_3$ jsou dvě mimoběžné přímky. \textbf{Odchylkou
    dvou mimoběžných přímek} $p,q$ nazýváme reálné číslo označené $|\sphericalangle
    p,q|$ a definované
    $$|\sphericalangle p,q| = |\sphericalangle p^\prime, q^\prime|,$$
    kde $p^\prime \parallel p \land q^\prime \parallel q,$ kde $p^\prime, q^\prime$
    jsou komplanární a různoběžné.
\end{definition}

\begin{definition}
    Nechť $p \subset \mathbb E_3$ je přímka a $\alpha \subset \mathbb E_3$ je
    rovina. \textbf{Odchylkou přímky $p$ od roviny $\alpha$} nazýváme
    reálné číslo označené $|\sphericalangle
    p,\alpha|$ a definované
    \begin{enumerate}[$i.$]
    \item je-li $p\parallel \alpha, |\sphericalangle p,\alpha|=0^\circ,$
    \item je-li $p\nparallel \alpha$, odchylka $p,\alpha$ je velikost ostrého nebo
        pravého úhlu,
        který svírají přímky $p,q$, kde $q$ je průsečnice rovin $\alpha, \beta$,
        přičemž $\beta \perp \alpha, p\in\beta.$
    \end{enumerate}
\end{definition}

\begin{definition}
    Nechť $\alpha,\beta \subset \mathbb E_3$ jsou dvě roviny. \textbf{Odchylkou
    rovin} $\alpha, \beta$ nazýváme reálné číslo označené $|\sphericalangle
    \alpha, \beta|$ a definované
    \begin{enumerate}[$i.$]
    \item je-li $\alpha\parallel \beta, |\sphericalangle \alpha,\beta|=0^\circ,$
   	\item je-li $\alpha \nparallel \beta$, odchylka $\alpha,\beta$ je velikost
        ostrého nebo pravého úhlu,
        který svírají přímky $p,q,$ kde $p \subset \alpha, q \subset \beta$ a obě
        přímky jsou kolmé k průsečnici rovin $\alpha, \beta.$
    \end{enumerate}
\end{definition}

\begin{definition}
    Nechť jsou dány nekomplanární body $A,B,C,D.$ Pak
  \textbf{čtyřstěn} je množina bodů ohraničená trojúhelníky $\triangle ABC, \triangle
  ABD, \triangle ACD, \triangle BCD.$
  \textbf{Pravidelný čtyřstěn} je tvořen čtyřmi stejnými rovnostrannými trojúhelníky.
\end{definition}

\begin{definition}
    Mějme v prostoru rovinu $\varrho,$ v ní konvexní mnohoúhelník $A_1\dots A_n$ a nechť
    $A_1^\prime$ je bod, který v rovině $\varrho$ neleží. Nechť $T\colon \mathbb E_3 \to
    \mathbb E_3$ je takové posunutí, že $A_1^\prime=T(A_1).$ Při tomto zobrazení se rovina
    $\varrho$ zobrazí na rovinu $\varrho^\prime,$ tyto dvě roviny jsou rovnoběžné. Množinu všech
    bodů $X$, všech úseček $BB^\prime$ takových, že $B\in A_1\dots A_n$ a $B^\prime$ je
    obraz bodu $B$ v posunutí $T$, nazýváme \textbf{hranolem}. Mnohoúhelníky
    $A_1A_2\dots A_n$ a $A^\prime_1A^\prime_2\dots A^\prime_n$ nazýváme \textbf{podstavami},
    rovnoběžníky $A_iA_{i+1}A^\prime_{i+1}A_i^\prime, i=1,\dots,n$, nazýváme
    \textbf{bočními stěnami hranolu}. Všechny boční stěny tvoří \textbf{plášť hranolu}.
    Podstavy spolu s bočními stěnami tvoří \textbf{stěny hranolu}. Úsečky $A_iA^\prime_i$,
    resp. $A_iA_{i+1}, A^\prime_iA^\prime_{i+1}$ se nazývají \textbf{boční}, resp. \textbf{podstavné hrany}
    hranolu. Body $A_1,A_2,\dots,A_n$ a $A^\prime_1,A^\prime_2,\dots,A^\prime_n$ se
    nazývají \textbf{vrcholy}. Je-li směr posunutí kolmý k~rovině podstavy, mluvíme
    o \textbf{hranolu kolmém}, v opačném případě jde o \textbf{hranol kosý}.
\end{definition}

\begin{definition}
    Hranol, jehož podstavy jsou rovnoběžníky, nazýváme \textbf{rovnoběžnostěn}.
\end{definition}

\begin{definition}
    Rovnoběžnostěn, jehož všechny stěny jsou pravoúhelníky (resp. čtverce) nazýváme
    \textbf{kvádr} (resp. \textbf{krychle}).
\end{definition}

\begin{definition}
    Mějme v prostoru rovinu $\varrho,$ v ní kruh $K$ ohraničený kružnicí $k$, na níž
    leží bod $A$ a nechť
    $A^\prime$ je bod, který v rovině $\varrho$ neleží. Nechť $T\colon \mathbb E_3 \to
    \mathbb E_3$ je takové posunutí, že $A^\prime=T(A).$ Označme $T(\varrho)=\varrho^\prime,
    T(k)=k^\prime, T(K)=K^\prime.$ Všechny body všech úseček $XX^\prime$, kde $X\in K$
    a $X^\prime$ je obraz bodu $X$, vytvoří \textbf{válec.} Omezíme-li se pouze na
    body $X$ ležící na kružnici $k$, dostaneme \textbf{plášť válce}. Kruhy $K,K^\prime$
    tvoří \textbf{podstavy válce}. Je-li směr posunutí $T$ kolmý k rovině $\varrho$,
    mluvíme o \textbf{kolmém válci}, v opačném případě o \textbf{kosém válci}.
\end{definition}

\begin{definition}
Mějme v prostoru rovinu $\varrho,$ v ní konvexní mnohoúhelník $A_1\dots A_n$ a nechť
$V$ je bod, který v rovině $\varrho$ neleží. Úsečky $VX$, kde $X$ jsou všechny body
mnohoúhelníka $A_1\dots A_n$, nazýváme \textbf{jehlanem}. Bod $V$ se nazývá
\textbf{hlavním vrcholem} jehlanu, jeho další \textbf{vrcholy} jsou $A_1,\dots,A_n$.
Mnohoúhelník $A_1\dots A_n$ je \textbf{podstava} jehlanu. Trojúhelníky
$A_iA_{i+1}V, i=1,\dots,n-1$ jsou \textbf{bočními hranami} jehlanu.
Úsečky $A_iA_{i+1}$ jsou \textbf{podstavnými hranami}. Boční stěny tvoří \textbf{plášť}
jehlanu. \textbf{Jehlan} se nazývá \textbf{pravidelný}, jestliže je jeho podstavou
pravidelný mnohoúhelník a~jeho hlavní vrchol má stejně velké vzdálenosti od
všech vrcholů podstavy.
\end{definition}

\begin{definition}
    Nechť je dán jehlan s hlavním vrcholem $V$ a podstavou $A_1\dots A_n$ v rovině
    $\varrho.$ Nechť $k \in \mathbb R, k \ne 1, k >0.$ Zaveďme $\mathscr H_{V,k}(A_1\dots A_n)
    =A_1^\prime\dots A_n^\prime.$ Těleso ohraničené podstavami $A_1\dots A_n$,
    $A_1^\prime\dots A_n^\prime$ a stěnami $A_iA_{i+1}A_{i+1}^\prime A_{i}^\prime$ se
    nazývá \textbf{komolý jehlan}.
\end{definition}

\begin{definition}
    Nechť je dán vrchol $V$ a kruhová podstava $K$ v rovině
    $\varrho.$ Množina všech bodů úseček $VX,$ kde $X\in K,$ se nazývá \textbf{kužel}.
    Jestliže $Y\in k,$ tvoří body úseček $VY$ \textbf{plášť} kužele, kruh $K$ je
    \textbf{podstavou kužele}, bod $V$ \textbf{vrcholem} kužele. Je-li
    $\overleftrightarrow{SV}\perp \varrho,$ nazývá se \textbf{kužel kolmý} (rotační).
\end{definition}

\begin{definition}
Nechť je dán kužel s hlavním vrcholem $V$ a podstavou $K$ v rovině
$\varrho.$ Nechť $k \in \mathbb R, k \ne 1, k >0.$ Zaveďme $\mathscr H_{V,k}(K)
=K^\prime.$ Množina všech bodů úseček $X\mathscr H(X),$ kde $X\in K$, je
\textbf{komolý kužel}.
\end{definition}

\begin{veta}
  Označme $V$ objem tělesa, $S$ jeho povrch, dále obsahy $S_{\textrm{pláště}}$, $S_{\textrm{podstavy}}$, $\mathbf{a},\mathbf{b},\mathbf{c}$ vektory stran, $a$, $b$, $c$ jejich délky. Pak se rovnají:

  \begin{center}
   \footnotesize
    \begin{tabularx}{\textwidth}{ l | l  l  l }

      \, & $V$ & $S$ & $S_{\text{pláště}}$ \\
      \hline
      prav. čtyřstěn & $\frac{\sqrt{2}}{12}a^3$ & $\sqrt{3}a^2$ & \\
      hranol & $S_{\textrm{podstavy}} \cdot v$ & $2S_{\textrm{podstavy}}+S_{\textrm{pláště}}$ & $o_{\textrm{podstavy}} \cdot v$ \\
      rovnoběžnostěn & $| ( \mathbf{a} \times \mathbf{b} ) \cdot \mathbf{c} | $ & \textrm{z vektorového součinu} & \, \\
      kvádr & $abc$ & $2(ab+bc+ca)$ & \, \\
      krychle & $a^3$ & $6a^2$ & \, \\
      válec & $S_{podstavy}\cdot v$ & $2S_{\textrm{podstavy}} + S_{\textrm{pláště}}$ & $o_{\textrm{podstavy}} \cdot v$ \\
      jehlan & $\frac{1}{3}\cdot S_{\textrm{podstavy}}\cdot v$ & $S_{\textrm{podstavy}} + S_{\textrm{pláště}}$ & \, \\
      komolý jehlan & $\frac{1}{3}(S_{\textrm{p1}} + \sqrt{S_{\textrm{p1}} S_{\textrm{p2}}} + S_{\textrm{p2}})\cdot v$ & \, & \,\\
      kužel & $\frac{1}{3}\cdot S_{\textrm{podstavy}}\cdot v$ & \, & \, \\
      komolý kužel & $\frac{1}{3}\pi(r_1^2 + r_1r_2 + r_2^2)\cdot v$ & \, & \,\\
      koule & $\frac{4}{3}\pi r^3$ & $4\pi r^2$ & \, \\
      kulová úseč & $\frac{\pi v^2}{3}(3r-v)$ & $\pi v (4r-v)$ & \, \\
      kulový vrchlík & \, & $2\pi r v$ & \,
    \end{tabularx}
  \end{center}
  \normalsize
\end{veta}

\begin{veta}[Cavalieriho princip]
    Nechť tělesa $T_1, T_2$ leží mezi dvěma rovinami $\varrho_1, \varrho_2$ a každá rovina
    $\varrho \parallel \varrho_1\parallel \varrho_2$ protne tělesa $T_1, T_2$ v konvexních
    rovinných útvarech s obsahy $P_1, P_2.$ Jestliže pro každou rovinu $\varrho$ platí
    $P_1=P_2$, mají $T_1$ a $T_2$ stejný objem.
\end{veta}

\begin{definition}
\textbf{Mnohostěn} je konvexní část prostoru, jejiž hranice je tvořena konečným počtem
mnohoúhelníků.
\end{definition}

\begin{veta}[Eulerova věta]
    Nechť $s$ je počet stěn, $h$ počet hran a $v$ počet vrcholů daného tělesa. Pak
    platí
    \begin{align*}
        s-h+v &=2,\\
        s+v &=h+2.
    \end{align*}
\end{veta}

\begin{pozn}[Pravidelné mnohostěny]\,
\begin{center}
\begin{tabular}{l|l|c|c|c|r}
    prav. mnohostěn & tvar stěny & $v$ & $s$ & $h$ & \, \\
    čtyřstěn        & rovnostranný trojúhelník & 4 & 4&6 & tetraedr \\
    šestistěn       & čtverec & 8 & 6&12 & hexaedr \\
    osmistěn        & rovnostranný trojúhelník & 6 & 8&12 & oktaedr \\
    dvanáctistěn    & pravidelný pětiúhelník & 20 & 12&30 & dodekaedr \\
    dvacetistěn     & rovnostranný trojúhelník & 12 & 20&30 & ikosaedr \\
\end{tabular}
\end{center}

\end{pozn}

\begin{priklad}
Odvoďte vztah pro povrch pláště rotačního kužele o výšce $v$ a poloměru
podstavy $r$.
\end{priklad}

\begin{reseni}
Plášť rotačního kužele tvoří kruhovou výseč o poloměru $s=\sqrt{v^2 + r^2}$ a obvodu (bez vyseknuté části)
$2\pi r$. I s vyseknutou částí by měl plášť obsah $\pi s^2$, ale ve skutečnosti je jeho obsah jen
$\frac{2\pi r}{2 \pi s}$-násobek tohoto čísla, tedy $\frac{2 \pi r }{2\pi s } \cdot \pi s^2=\pi r s = \pi r \sqrt{v^2 + r^2}$.
\begin{figure}[h]
\begin{subfigure}{.5\textwidth}
	\centering
	\begin{tikzpicture}[scale=1.2, line cap=round, line join=round]
    % Definice hlavních bodů
    \coordinate (V) at (0, 3.5);  % Vrchol kužele
    \coordinate (S) at (0, 0);    % Střed podstavy

    % Zadní část elipsy podstavy (čárkovaně)
    \draw[dashed, gray] (2,0) arc (0:180:2 and 0.6);
    % Přední část elipsy podstavy (plná čára)
    \draw (-2,0) arc (180:360:2 and 0.6);

    % Plášť kužele (stranové přímky)
    \draw (-2,0) -- (V) -- (2,0);

    % Vnitřní konstrukce (výška v a poloměr r)
    \draw[dashed, blue] (V) -- (S);
    \draw[dashed, blue] (S) -- (2,0);

    % Značka pravého úhlu u středu podstavy
    \draw[blue] (0, 0.2) -- (0.2, 0.2) -- (0.2, 0);

    % Popisky vrcholů a rozměrů
    \node[above] at (V) {$V$};
    \node[below] at (S) {$S$};
    \node[right] at (1, 1.75) {$s$};
    \node[above] at (1, 0) {$r$};
    \node[left] at (0, 1.75) {$v$};

    % Bod na obvodu podstavy (např. A)
    \coordinate (A) at (2,0);
    \node[right] at (A) {$A$};
    \draw[fill] (A) circle (1pt);
    \draw[fill] (V) circle (1pt);
    \draw[fill] (S) circle (1pt);
	\end{tikzpicture}
	\caption{Rotační kužel}
\end{subfigure}
	\begin{subfigure}{.5\textwidth}
		\centering
		\begin{tikzpicture}[scale=1, line cap=round, line join=round]
    % Parametry pro kreslení (strana s a úhel α ve stupních)
    \def\s{2.5}       % Poloměr výseče (strana kužele s)
    \def\alpha{300} % Úhel výseče (zde zvolen pro názornost jako půlkruh 180°, lze upravit)

    % Vrchol výseče (odpovídá vrcholu kužele V)
    \coordinate (V) at (0,0);

    % Vybarvení plochy pláště
    \draw[fill=gray!15, draw=none] (V) -- (0:\s) arc (0:\alpha:\s) -- cycle;

    % Obvodové hrany výseče
    \draw[thick] (V) -- (0:\s);
    \draw[thick] (V) -- (\alpha:\s);
    \draw[thick] (0:\s) arc (0:\alpha:\s);

    % Popisky
    \node[above] at (V) {$V$};

    % Kóty a popisy stran
    \node[above] at ({0.5*\s}, 0) {$s$};

    \draw[fill] (V) circle (1.5pt);
		\end{tikzpicture}
		\caption{Rozvinutý plášť rotačního kužele}
	\end{subfigure}
	\caption{Odvození vzorce pro obsah pláště rotačního kužele}
\end{figure}
\end{reseni}

\begin{definition}
    O tělesu $M$ v prostoru říkáme, že má $p$ za osu rotace a že je \textbf{rotačním
    tělesem}, jestliže se zobrazí samo na sebe při každém otočení kolem přímky $p.$
\end{definition}

\begin{pozn}
Příklady rotačních těles:
\begin{itemize}
\item \textbf{rotační válec} -- pravoúhelník otáčený kolem své strany;
\item \textbf{rotační kužel} -- pravoúhlý trojúhelník otáčený kolem odvěsny;
\item \textbf{koule} -- kruh otáčený kolem svého průměru, hranice koule se nazývá \textbf{kulová plocha};
\item \textbf{torus} -- kruh kolem osy, která leží mimo něj.
\end{itemize}
\end{pozn}
