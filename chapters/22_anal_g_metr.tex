...
\begin{definition}
    ...
    podprostorů} $A,B$ nazveme nezáporné reálné číslo $\varrho(A,B)$ definované jako
    $$\varrho(A,B) = \min \{ |XY|; X\in A, Y\in B \},$$
...
\begin{veta}
    ...
    $$\varrho(A,p)=\frac{|aa_1+ba_2+c|}{\sqrt{a^2+b^2} }.$$
\end{veta}
...
Určete $\varrho(A,p), A[1,0,1], p=\left \{ [2-t,t,0]; t \in \mathbb R \right \} $.
\end{priklad}
...
$\varrho(A,p)$ je pak $\left |\overrightarrow{AA_0}\right |.$
...
\begin{veta}\label{dverov}
    ...
    $$\varrho(\alpha,\beta)=\frac{|e-d|}{\sqrt{a^2+b^2+c^2} }.$$
\end{veta}
