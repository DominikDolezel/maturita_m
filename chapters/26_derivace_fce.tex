...
\begin{pozn}
    Hodnota derivace v daném bodě je směrnice tečny funkce v tomto bodě.
\end{pozn}
...
\begin{definition}
Nechť existuje vlastní derivace funkce $f(x)$ pro všechna $x\in M, M\subset \mathcal D(f).$
...
\begin{veta}[Bolzanova věta]
Má-li funkce $f$ v bodě $a$ vlastní derivaci, je v tomto bodě spojitá.
\end{veta}

\begin{pozn}
    Má-li funkce $f\colon M\to \mathbb R$ na množině $M\subset \mathbb R$ vlastní derivaci
    v každém bodě $x\in M,$ pak zobrazení $f^\prime\colon M\to \mathbb R,$ které přiřadí
...
\item $(\tg x)^\prime = \frac{1}{\cos^2 x}, x \in \mathbb R \smallsetminus \bigcup\limits_{k\in \mathbb Z}\left \{ \frac{\pi}{2}+k\pi \right \}$;
\item $(\cotg x)^\prime = -\frac{1}{\sin^2 x}, x \in \mathbb R \smallsetminus \bigcup\limits_{k \in \mathbb Z}\left \{ k\pi \right \}$;
...
Nechť funkce $f\colon x=f(y)$ je spojitá a ryze monotónní na intervalu $I.$ Nechť $y_0$
je vnitřní bod intervalu $I$ a nechť má $f$ v $y_0$ derivaci $f^\prime(y_0).$
Pak inverzní funkce $f^{-1}\colon y=f^{-1}(x)$ má v bodě $x_0=f(y_0)$ derivaci a platí
$$
...
$$F^\prime (x,y)=\frac{\partial F}{\partial x}(x, y) + \frac{\partial F}{\partial y}(x, y)\cdot y^\prime.$$
\end{veta}
...
\begin{reseni}
  Derivujeme funkci danou implicitně: $f(x,y)^\prime\colon 2x+2yy^\prime=0.$
\end{reseni}

\begin{veta}[Cauchyho--Bolzanova věta]
Nechť funkce $f$ má následující vlastnosti:
\begin{enumerate}[i.]
\item je spojitá na uzavřeném intervalu $\left<  a,b \right> $,
\item platí $f(a)\cdot f(b)<0$.
\end{enumerate}
Pak existuje alespoň jedno číslo $x_0\in (a,b)$ takové, že $f(x_0)=0$.
\end{veta}
...
\begin{reseni}
	Zlomek rozložíme na součin
	$$
	    f(x) = \frac{(1-x)(1+x)}{x},
	$$
	určíme nulové body a zaneseme je na číselnou osu. V nulových bodech se mění znaménko.
	Dále funkce není definovaná v bodě 0.
	\begin{figure}[!ht]
	\centering
	\begin{circuitikz}
	...
	\node [font=\fontsize{13.1pt}{17.0pt}\selectfont, inner xsep=0.080cm, inner ysep=0.085cm, rounded corners=0.000cm] at (11.25,6.375) {1};
	...
	\node [font=\fontsize{13.1pt}{17.0pt}\selectfont, inner xsep=0.080cm, inner ysep=0.085cm, rounded corners=0.000cm] at (8.125,7.5) {$+$};
	\draw (12.5,7) to[short] (7.5,7);
	\begin{scope}[transparency group, opacity=1]
	...
	\draw (10,7) to[short, -o] (10,7) ;
	\node [font=\fontsize{13.1pt}{17.0pt}\selectfont, inner xsep=0.080cm, inner ysep=0.085cm, rounded corners=0.000cm] at (9.375,7.5) {$-$};
	\end{circuitikz}
	\caption{Znaménka $f(x)$ na číselné ose}
	\end{figure}
	\par Funkce $f(x)$ je tedy kladná na intervalech $\left (-\infty, -1 \right >$ a $\left( 0, 1 \right > $
	a záporná na intervalech $\left < -1, 0 \right ) $ a $\left < 1, \infty \right ) $.
\end{reseni}
