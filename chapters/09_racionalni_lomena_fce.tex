...
\end{equation*}
Hyperbola je tedy posunutá o dva nahoru a o jedna doleva. Přesvědčit se můžeme v obrázku \ref{linlomfce}.
\end{reseni}

\begin{pozn}
  Pro lineární lomenou funkci $f$ platí
  \begin{itemize}
...
\begin{definition}
    ...
    Pak funkci $F\colon y=\frac{P(x)}{Q(x)}$ nazýváme \textbf{racionální lomenou funkcí}.
\end{definition}
...
\begin{definition}
    ...
    se nazývá
    \begin{enumerate}[$i.$]
        \item \textbf{ryze lomená}, pokud $\text{st}\left (P(x) \right ) <
        \text{st}\left (Q(x) \right )$,
       	\item \textbf{neryze lomená}, pokud $\text{st}\left (P(x) \right ) \geq
        \text{st}\left (Q(x) \right )$.
    \end{enumerate}
\end{definition}
...
\begin{reseni}
  ...
  Řešení vyčteme jednoduše z osy. Vidíme, že $Q(x)$ je větší nebo roven nule v oboru
  $$
...
\begin{priklad}
Rozložte zlomek $y=\frac{x-4}{x^2-5x+6}$ na parciální zlomky.
\end{priklad}

\begin{reseni}
Nejprve rozložíme jmenovatele na součin. Platí
$$
